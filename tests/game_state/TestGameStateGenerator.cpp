...
template<> template<>
void NotifierMockWrapper<int, float, double>::notify<int>(size_t id, size_t frame) {
    mock.get().notify_int(id, frame);
}
template<> template<>
void NotifierMockWrapper<int, float, double>::notify<float>(size_t id, size_t frame) {
    mock.get().notify_float(id, frame);
}
template<> template<>
void NotifierMockWrapper<int, float, double>::notify<double>(size_t id, size_t frame) {
    mock.get().notify_double(id, frame);
}
template<> template<>
IEventApplier<size_t, size_t>& NotifierMockWrapper<int, float, double>::get_event_source<int>() {
    return mock.get().get_event_source_int();
}
template<> template<>
IEventApplier<size_t, size_t>& NotifierMockWrapper<int, float, double>::get_event_source<float>() {
    return mock.get().get_event_source_float();
}
template<> template<>
IEventApplier<size_t, size_t>& NotifierMockWrapper<int, float, double>::get_event_source<double>() {
    return mock.get().get_event_source_double();
}

BOOST_AUTO_TEST_CASE(GameStateGenerator_regularCase) {
    ...
    gs_defs::GameStateGenerator gs(queues,
                                   NotifierMockWrapper<int, float, double>{notifier_mock});
